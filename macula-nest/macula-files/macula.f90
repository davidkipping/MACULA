...
!          v1.4 - Computes linear minimization of U(:) terms
!                 Replaces Box(m,i) functions with ikronecker(m,mval(i))
!                 where mval(i) is computed like an epoch in main.f90
!                 Uses Tstart/Tend arrays read-in earlier from params.f90
!              
! INPUTS:
!
! ------------------------------------------------------------------------------
...
! ------------------------------------------------------------------------------
! B(m) 		= Theta_inst(1,m)	! Blend factor for m^th data set
! ------------------------------------------------------------------------------
...
SUBROUTINE macula(t,Fobs,sigma,fobswei,sigfobswei,mval,&		!Inputs
                  ndata,Nspot,mmax,derivatives,temporal,TdeltaV,&	!Controls
...
implicit none
 ...
 INTEGER, PARAMETER :: pinst = 1
 INTEGER, PARAMETER :: pLD = 5
 ! === INPUTS ===
 INTEGER, INTENT(IN) :: ndata, mmax, Nspot
 REAL(8), DIMENSION(ndata), INTENT(IN) :: t, Fobs, sigma, fobswei, sigfobswei
 INTEGER, DIMENSION(ndata), INTENT(IN) :: mval
 ...
 REAL(8), PARAMETER :: LogTwoPi = 1.8378770664093453
 ...
 REAL(8), DIMENSION(pLD,Nspot,ndata) :: Upsilon, w
 REAL(8), DIMENSION(Nspot,ndata) :: Psi, Xi
 REAL(8), DIMENSION(Nspot,ndata) :: q, A
 REAL(8) :: Fab0
 REAL(8), DIMENSION(ndata) :: Fab, Finter
 REAL(8), DIMENSION(mmax) :: obsmod1, mod2
 REAL(8), DIMENSION(ndata) :: Fmod, deltaratio
 REAL(8), INTENT(OUT) :: loglike
 ! === SECTION 4 ===
 REAL(8), DIMENSION(pLD,pstar+pspot*Nspot+pinst*mmax) :: dc, dd
 REAL(8), DIMENSION(mmax,pstar+pspot*Nspot+pinst*mmax) :: dB
 REAL(8), DIMENSION(Nspot,pstar+pspot*Nspot+pinst*mmax) :: dfspot
 ! === SECTION 5 ===
 REAL(8), DIMENSION(Nspot,ndata) :: eta
 ...
 ! === SECTION 7 ===
 REAL(8), DIMENSION(pLD,Nspot,ndata,pstar+pspot*Nspot+pinst*mmax) :: dUpsilon, dw
 ...
 REAL(8), DIMENSION(pstar+pspot*Nspot+pinst*mmax) :: dFab0, dloglike
 ! === SECTION 8 ===
 REAL(8), DIMENSION(Nspot,ndata) :: dalphadt, dbetadt, dzetanegdt, dzetaposdt
 REAL(8), DIMENSION(Nspot,ndata) :: dqdt, dAdt
 REAL(8), DIMENSION(pLD,Nspot,ndata) :: dwdt, dUpsilondt
 REAL(8), DIMENSION(mmax,ndata) :: dFtildedt
 REAL(8), DIMENSION(ndata) :: dFabdt
 REAL(8), DIMENSION(ndata) :: dFmoddt
 ! === SECTION 9 ===
 REAL(8), DIMENSION(ndata,pstar) :: dFmod_star
 REAL(8), DIMENSION(ndata,pspot,Nspot) :: dFmod_spot
 REAL(8), DIMENSION(ndata,pinst,mmax) :: dFmod_inst
 ...
 ! Finter
 DO i=1,ndata
   Finter(i) = Fab(i)/(Fab0*B(mval(i))) + (B(mval(i))-1.0D0)/B(mval(i))
 END DO
 ...
 DO m=1,mmax
   DO i=1,ndata
     IF( mval(i) .EQ. m ) THEN
       obsmod1(m) = obsmod1(m) + fobswei(i)*Finter(i)
       mod2(m) = mod2(m) + Finter(i)**2*sigfobswei(i)
       U(m) = obsmod1(m)/mod2(m)
     END IF
   END DO
 END DO
 ...
 ! Fmod
 DO i=1,ndata
   Fmod(i) = U(mval(i))*Finter(i)
 END DO

 IF( TdeltaV ) THEN
   DO i=1,ndata
     deltaratio(i) = Fab0/( Fab(i)*B(mval(i)) )
   END DO
 ELSE
   deltaratio(:) = 1.0D0
 END IF
 ...
 loglike = 0.0D0
 DO i=1,ndata
   !write(*,*) DABS( (Fobs(i)-Fmod(i))/(sigma(i)) )
   loglike = loglike + ( (Fobs(i)-Fmod(i))/(sigma(i)) )**2
 END DO
 !write(*,*) 'chi2 = ',loglike
 loglike = -0.5D0*ndata*LogTwoPi - DLOG( SUM(sigma(:)) ) - 0.5D0*loglike
 IF( IsNan(loglike) ) THEN
   !write(*,*) loglike
   loglike = -HUGE(1.0D0)
 END IF
 ...
 dB(:,:) = 0.0D0
 DO m=1,mmax
   ! d(B_m)/d(Theta_j) for j=pstar+pspot*Nspot+m => Theta_j=B_m
   dB(m,pstar+pspot*Nspot+m) = 1.0D0
 END DO
 ...
 DO j=1,jmax
   DO i=1,ndata
     DO m=1,mmax
       dFtilde(m,i,j) = U(m)*( B(m)*Fab0*dFab(i,j) - B(m)*Fab(i)*dFab0(j) &
		        + Fab0*(Fab0-Fab(i))*dB(m,j) )
       dFtilde(m,i,j) = ( ikronecker(m,mval(i))/(Fab0**2*B(m)**2) )*dFtilde(m,i,j)
     END DO
   END DO
 END DO
 ...
 ! Derivatives of loglike
 dloglike(:) = 0.0D0
 DO j=1,jmax
   DO i=1,ndata
     dloglike(j) = dloglike(j) + ( ( dFmod(i,j)*(Fobs(i)-Fmod(i)) )/sigma(i)**2 )
   END DO
 END DO
 IF( IsNan(loglike) ) THEN
   dloglike(:) = 0.0D0
 ...
 DO i=1,ndata
   DO m=1,mmax
     dFtildedt(m,i) = ( (U(m)*ikronecker(m,mval(i)))/(B(m)*Fab0) )*dFabdt(i)
   END DO
 END DO
...
! =========================== FUNCTION: IKRONECKER ==============================
! ==============================================================================
REAL(8) FUNCTION ikronecker(x,y)
