...
SUBROUTINE macula(t,&							!Inputs
                  ndata,Nspot,mmax,derivatives,temporal,TdeltaV,plot,&	!Controls
                  Theta_star,Theta_spot,Theta_inst,&			!System parameters
                  Tstart,Tend,Fmod,dFmoddt,cosalpha,deltaratio)				!Outs
...
implicit none
 ...
 LOGICAL, INTENT(IN) :: plot	    ! Produce plotting data?
 ! === VARIABLES ===
 INTEGER :: i, j, k, l, m, n
 INTEGER :: jmax
 ...
 REAL(8), DIMENSION(Nspot,ndata) :: alpha, sinalpha
 REAL(8), DIMENSION(Nspot,ndata), INTENT(OUT) :: cosalpha
 ...
 REAL(8), DIMENSION(ndata), INTENT(OUT) :: deltaratio
 ! === SECTION 4 ===
 ...
 ! Finter
 DO i=1,ndata
   Finter(i) = Fab(i)/(Fab0*Bi(i)) + (Bi(i)-1.0D0)/Bi(i)
 END DO
 ...
 IF( plot ) THEN
   open(unit=13,file='macula-lc.dat')
   DO i = 1,ndata
     write(13,*) t(i),Fmod(i),deltaratio(i),Phi0(:),Lambda(:,i),alpha(:,i)
   END DO
   close(13)
 END IF
